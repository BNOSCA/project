...
function authErrorMessage(error: unknown) {
  ...
  if (code.includes('operation-not-allowed')) return 'Firebase 尚未啟用 Email／密碼登入方式。'
  if (code.includes('api-key-not-valid')) return 'Firebase API key 無效，請確認前端環境設定。'
  if (code.includes('network-request-failed')) return '無法連線到 Firebase，請確認網路或瀏覽器外掛設定。'
  if (code.includes('too-many-requests')) return '嘗試次數過多，請稍後再試。'
  if (code.includes('timeout')) return 'Firebase 驗證逾時，請確認網路與 Firebase Authentication 是否已啟用。'
